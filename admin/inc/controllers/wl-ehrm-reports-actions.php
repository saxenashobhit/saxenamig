...
class ReportAjaxAction {
    ...
    public static function get_reports() {
        ...
        if ( isset( $_POST['staff_id'] ) && isset( $_POST['month'] ) && isset( $_POST['type'] ) ) {
			...
			foreach ( $breaksdata as $key1 => $breaks ) {
				if( $attendence['date'] == $breaks['date'] ){
                ...
                if ( ! empty( $attendences ) && ( $type == 'all' || $type == 'attend' ) ) {
                    foreach ( $attendences as $key => $attendence ) {
                        if ( $attendence['date'] == $date && $attendence['staff_id'] == $staff_id && ! empty( $attendence['office_in'] ) && ! in_array( $date, $date_arr ) ) {
                            // echo $attendence['office_in']; 
                            // echo $date;
                            // die();
                            $late          = $attendence['late'];
                            $working_hours = EHRMHelperClass::ehrm_daily_working_hours( $staff_id, $date );
                            // echo "<pre>"; var_dump($working_hours); echo "</pre>"; die();
							// echo $working_hours = $attendence['working_hour'];
							//$working_hours = EHRMHelperClass::get_time_difference( $working_hours, $breaksdata[$key]['break_time'] );
							
							//$working_hours = "page 2";
							//$working_hours = EHRMHelperClass::get_time_difference( $working_hours, $duration_arr );
							//$working_hours = $breaksdata[$key]['break_time'];
							
							//$duration_arr = ReportAjaxAction::break_time( $duration_arr );
							//$working_hours = EHRMHelperClass::get_time_difference( $working_hours, $duration_arr );
							...
							if ( ! empty ( $all_breaks ) ) {
								...
								foreach ( $all_breaks as $key => $breaks ) {
									$duration_arr = array();
									if ( $breaks['date'] == $attendence['date'] && $breaks['user_id'] == $staff_id ) {

										array_push( $duration_arr, $breaks['break_time'] );
										$nos++;
										//var_dump($duration_arr);
									}
                                    // echo "<pre>"; var_dump(ReportAjaxAction::break_time( $duration_arr )); echo "</pre>"; die();
                                    ReportAjaxAction::break_time( $duration_arr );
									$working_hours = EHRMHelperClass::get_time_difference( $working_hours, ReportAjaxAction::break_time( $duration_arr ) );
								}
							...
							if ( ! empty ( $duration_arr ) ) {
									$duration_arr = ReportAjaxAction::break_time( $duration_arr );
									$work_hour 	  = strtotime( $work_hour ) - strtotime( $duration_arr );
									$work_hour 	  = date( "H:i:s", ( $work_hour ) );
									$working_hours = EHRMHelperClass::get_time_difference( $working_hours, ReportAjaxAction::break_time( $duration_arr ) );
								}
    ...
    public static function display_salary() {
        ...
        if ( isset( $_POST['staff_id'] ) && isset( $_POST['month'] ) && isset( $_POST['type'] ) ) {
            ...
            $project_data_for_user = EHRMHelperClass::ehrm_project_based_user_info($staff_id);
            // echo "<pre>"; var_dump($project_data_for_user); echo "</pre>";
            ...
            if ( ! empty( $staffs_data ) ) {
                foreach ( $staffs_data as $staff_key => $staffs ) {
                    if ( $staffs['ID'] == $staff_id ) {
                        ...
                        for ( $i = 0; $i < $size; $i++ ) {
                            $leaves = $leaves + (int)$all_leaves[$i];
                        }
            ...
            if( $no_of_projects_completed > 0 ) { 
                ...
                foreach( $final_data as $project_title => $project_title_cost ) {
                    $html .= '<tr>
                                <td>'.$j.'</td>
                                <td>'.$project_title.'</td>
                                <td class="right-td"><span class="info-value-span">'.esc_html( EHRMHelperClass::get_currency_position_html( $project_title_cost ) ).'</span></td>
                            </tr>';
                            $j++;
                }
                $html .= '<tr>
                            <td colspan="2">'.esc_html__( 'Your Total Salary', '"employee-&-hr-management"' ).'</td>
                            <td class="right-td"><span class="info-value-span">'.esc_html( EHRMHelperClass::get_currency_position_html( $total_salary ) ).'</span></td>
                    </tr>';
                    $html .= '</tbody>
                    </table>
                </div>
